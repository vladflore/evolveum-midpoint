...
The resource wizard for credential mappings is similar to the xref:admin-gui/resource-wizard/object-type/activation/index.adoc[wizard used for activation configuration].
...
image::admin-gui/resource-wizard/object-type/credentials/credential-mapping-setup-accounts-configure-menu.webp["Configuration menu in the resource account list as an entry point to the credential mapping rules wizard"]
...
image::admin-gui/resource-wizard/object-type/credentials/credential-outbound-mapping.webp["Outbound credential mappings overview with one active mapping to generate password"]
...
* Set *strength* based on the mapping purpose. See the xref:expressions/mappings/index.adoc#_mapping_strength[explanation of the mapping strength options].
...
* For [.nowrap]#icon:arrow-right-to-bracket[] *Inbound*# mappings:
    ** *Source* specifies an attribute on the resource side from which to take credentials.
        This is typically defined in the xref:resources/resource-configuration/index.adoc[resource schema] and needs not to be specified manually here.
...
* For [.nowrap]#icon:arrow-right-from-bracket[] *Outbound*# mappings:
    ...
    ** If you cannot rely on the resource schema definition for password target mapping because your users have two passwords, for instance,
        you need to map the secondary password to the appropriate resource attribute using general outbound xref:admin-gui/resource-wizard/object-type/mapping/index.adoc[mapping rules].
...
* *Authoritative* option defines whether the mapping "tolerates" changes by other means.
    ** Refer to xref:expressions/mappings/index.adoc#authoritative[Mappings (authoritative)] for details.
* *Exclusive* option ensures only one mapping can target the particular attribute.
    ** Refer to xref:expressions/mappings/index.adoc#exclusive[Mappings (exclusive)] for details.
* *Channel* option provides a way to restrict the mapping applicability only to the context of certain channels.
    ** Refer to xref:expressions/mappings/index.adoc#channel-constraints[Mappings (channel constraints)] for details.
...
We suggest you change it to _Proposed_ and xref:admin-gui/resource-wizard/wizard-existing-resource/index.adoc#simulated-tasks[test your configuration] first.
====
...
* xref:concepts/object-lifecycle/index.adoc[Object Lifecycle]: Gain a deeper understanding of object lifecycle management in midPoint.
* xref:admin-gui/admin-gui-config/index.adoc#wizard-panels[Admin GUI configuration]: See configuration options for certain wizard panels and the GUI in general.
* xref:security/credentials/index.adoc[Credentials]
* xref:expressions/index.adoc[Expressions]
* xref:simulation/index.adoc[Simulation]
* xref:resources/resource-schema/index.adoc[Resource Schema]
* xref:resources/resource-configuration/index.adoc[Resource Configuration]
* xref:resources/resource-configuration/schema-handling/index.adoc[Schema Handling]
